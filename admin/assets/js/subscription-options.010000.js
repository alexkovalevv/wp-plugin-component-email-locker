...
if( !window.bizpanda.subscriptionOptions ) {
	...
	window.bizpanda.subscriptionOptions = {
		...
		init: function(item) {
			...
			$("#opanda_subscribe_allow_social").change(function() {
				...
				if( value ) {
					$("#social-buttons-options").fadeIn();
				} else {
					$("#social-buttons-options").hide();
				}
			}).change();
		...
		getSubscriptionOptions: function() {
			...
			var connectButtons = [];
			if( $("#factory-checklist-opanda_subscribe_social_buttons-facebook").is(":checked") ) {
				connectButtons.push('facebook');
			}
			if( $("#factory-checklist-opanda_subscribe_social_buttons-google").is(":checked") ) {
				connectButtons.push('google');
			}
			if( $("#factory-checklist-opanda_subscribe_social_buttons-linkedin").is(":checked") ) {
				connectButtons.push('linkedin');
			}

			if( $("#factory-checklist-opanda_subscribe_social_buttons-vk").is(":checked") ) {
				connectButtons.push('vk');
			}
			...
			options.connectButtons.vk = {
				appId: window.opanda_vk_app_id,
				actions: ['subscribe']
			};
