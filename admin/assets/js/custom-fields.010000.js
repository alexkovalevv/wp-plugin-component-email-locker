...
(function($){
    ...
    if( !onp_lang('ru_RU') ) {
        $.widget("opanda.fieldsEditor", {
            ...
            _typesList: [
                ...
                },
                {
                    ...
                    title: 'Phone',
                    editorName: 'text',
                    placeholder: window.bizpanda.res['phone-field-placeholder']
                },
                {name: 'url', title: 'Website/URL', editorName: 'text', placeholder: 'http://'},
                {
                    ...
                    title: 'Birthday',
                    editorName: 'text',
                    placeholder: window.bizpanda.res['birthday-field-placeholder']
                },
                {name: 'integer', title: 'Integer', editorName: 'integer'},
                {name: 'hidden', title: 'Hidden', editorName: 'hidden', helper: false},
            ...
            initOptionsEditor: function ($fieldRow, $container) {
                ...
                if ('dropdown' === editorName) {
                    ...
                    if (fieldOptions.password) {
                        $container.find(".opanda-is-password-input").attr('checked', 'checked');
                    }
                ...
                } else if ('hidden' === editorName) {
                    $container.find(".opanda-value-input").val(fieldOptions.value);
            ...
            getOptionsEditorValuesToSave: function ($container, editorName) {
                ...
                if ('text' === editorName) {
                    ...
                    values.password = $container.find(".opanda-is-password-input").is(":checked");
                } else if ('integer' === editorName) {
                    values.iconPosition = $container.find(".opanda-icon-position-input").val();
                    values.placeholder = $container.find(".opanda-placeholder-input").val();
                    values.title = $container.find(".opanda-title-input").val();
                    ...
                    values.value = $container.find(".opanda-value-input").val();
                } else if ('label' === editorName) {
            ...
            createField: function (data) {
                ...
                if (data) {
                    ...
                    var $mappingInput = $blank.find(".opanda-mapping-input");
                    $mappingInput.val(data.mapOptions.id);
                    $mappingInput.data('value', data.mapOptions.id);
                    var $typeInput = $blank.find(".opanda-type-input");
                    $typeInput.val(data.fieldOptions.type);
                    $typeInput.data('value', data.fieldOptions.type);
            ...
            loadSaved: function () {
                ...
                if (strjson) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if ('email' === data[i].serviceOptions.sysname) {
                            hasEmail = true;
                        }
        ...
        $.widget("opanda.fieldsEditor", {
             ...
             */
            _typesList: [
                {name: 'text', title: 'Текст', editorName: 'text'},
                {
                    name: 'email',
                    title: 'Email',
                    editorName: 'text',
                ...
                {name: 'url', title: 'Вебсайт/URL', editorName: 'text', placeholder: 'http://'},
                {
                    name: 'birthday',
                    title: 'День рожденья',
                    editorName: 'text',
                ...
                {name: 'integer', title: 'Число', editorName: 'integer'},
                {name: 'hidden', title: 'Скрытое', editorName: 'hidden', helper: false},
                {name: 'dropdown', title: 'Выпадающее меню', editorName: 'dropdown'},
                {name: 'checkbox', title: 'Флажок', editorName: 'checkbox'},
                {name: 'date', title: 'Дата', editorName: 'text'},
                {name: 'unsupported', title: window.bizpanda.res['unsupported'], hide: true},
                {name: 'separator', title: 'Разделитель', editorName: 'separator', helper: true, hide: true},
                {name: 'label', title: 'Текстовая метка', editorName: 'label', helper: true, hide: true},
                {name: 'html', title: 'HTML код', editorName: 'html', helper: true, hide: true}
            ],
            /**
             * Font Awesome Icons
             */
            ...
            _create: function () {
                ...
                $items.addClass('opanda-loading');
                var $selectors = this.getLazySelectors();
                this.lockSelects($selectors, "[ - загрузка - ]");
                this.initColumns();
                this.initHints();
                // saves values on chaning on any of input elements
                this._$fieldsEditor.on("change", ".opanda-choices-editor, input, select, textarea", function () {
            ...
            onUpdatingMappingSelector: function ($fieldRow, onChange) {
                ...
                if (this.isEmailField($fieldRow)) {
                    ...
                    mapOptions = {mapTo: 'text', id: "fullname", labelTitle: 'Полное имя'};
                    premissions = {can: {changeType: true, changeReq: false}};
                } else if (value === 'separator') {
                    fieldOptions = {req: false, type: 'separator'};
                    mapOptions = {mapTo: 'separator', id: "separator"};
    ...
    $.widget( "opanda.choicesEditor", {
        ...
        addChoice: function( value ) {
            ...
            if ( value ) $blank.find('.opanda-choise-value-input').val(value);
