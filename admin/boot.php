<?php
	...
	add_filter('bizpanda_email-locker_type_metaboxes', 'opanda_email_locker_metaboxes', 10, 1);
	...
	function opanda_email_locker_activation($plugin, $helper)
	{
		...
		$helper->addPost('opanda_default_email_locker_id', array(
			'post_type' => OPANDA_POST_TYPE,
			'post_title' => __('Email Locker (default)', 'bizpanda-email-locker'),
			'post_name' => 'opanda_default_email_locker'
		), array(
			'opanda_item' => 'email-locker',
			'opanda_header' => __('This Content Is Only For Subscribers', 'bizpanda-email-locker'),
			'opanda_message' => __('Please subscribe to unlock this content. Enter your email to get access.', 'bizpanda-email-locker'),
	...
	add_action('bizpanda_after_activation', 'opanda_email_locker_activation', 10, 2);
	...
	function opanda_register_email_locker_themes($item)
	{
		...
		if( !onp_build('free') ) {
			...
			OPanda_ThemeManager::registerTheme(array(
				...
				'previewHeight' => 450,
				'hint' => sprintf(__('This theme is available only in the <a href="%s" target="_blank">premium version</a> of the plugin', 'bizpanda-email-locker'), opanda_get_premium_url(null, 'themes')),
				'items' => array('signin-locker', 'email-locker')
			));
	...
	add_action('bizpanda_register_themes', 'opanda_register_email_locker_themes');
	...
	add_action('bizpanda_help_page_what-is-email-locker', 'opanda_help_page_what_is_email_locker');
	...
	add_action('bizpanda_help_page_usage-example-email-locker', 'opanda_help_page_usage_example_email_locker');
	...
	function opanda_email_locker_stats_screens($screens)
	{
		...
		$screens = array(
			...
			'summary' => array(
				'title' => __('<i class="fa fa-search"></i> Summary', 'bizpanda-email-locker'),
				'description' => __('The page shows the total number of unlocks for the current locker.', 'bizpanda-email-locker'),
			...
			'profits' => array(
				'title' => __('<i class="fa fa-usd"></i> Benefits', 'bizpanda-email-locker'),
				'description' => __('The page shows benefits the locker brought for your website.', 'bizpanda-email-locker'),
			...
			'channels' => array(
				'title' => __('<i class="fa fa-search-plus"></i> Channels', 'bizpanda-email-locker'),
				'description' => __('The page shows which ways visitors used to unlock the content.', 'bizpanda-email-locker'),
			...
			'bounces' => array(
				'title' => __('<i class="fa fa-sign-out"></i> Bounces', 'bizpanda-email-locker'),
				'description' => __('The page shows major weaknesses of the locker which lead to bounces. Hover your mouse pointer on [?] in the table, to know more about a particular metric.', 'bizpanda-email-locker'),
			...
			'skips' => array(
				'title' => __('<i class="fa fa-tint"></i> Skips', 'bizpanda-email-locker'),
				'description' => __('The chart shows how many users skipped the locker by using the Timer or Close Icon, comparing to the users who unlocked the content.', 'bizpanda-email-locker'),
	...
	add_filter('bizpanda_email-locker_stats_screens', 'opanda_email_locker_stats_screens', 10, 1);