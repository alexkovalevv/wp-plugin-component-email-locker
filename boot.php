<?php
	...
	if( is_admin() ) {
		require BIZPANDA_EMAIL_LOCKER_DIR . '/admin/boot.php';
	}

	global $bizpanda;

	load_textdomain('bizpanda-email-locker', BIZPANDA_EMAIL_LOCKER_DIR . '/langs/bizpanda-email-locker-' . get_locale() . '.mo');
	...
	function opanda_register_email_locker($items)
	{
		...
		if( onp_build('free') ) {
			...
			$items['email-locker'] = array(
				'name' => 'email-locker',
				'title' => __('Email Locker', 'bizpanda-email-locker'),
				'help' => opanda_get_help_url('what-is-email-locker'),
				'description' => __('<p>Asks visitors to subscribe to unlock some value content.</p><p>Perfect way to build your mailing list without any extra costs or efforts.</p>', 'bizpanda-email-locker'),
				'type' => 'free',
	...
	add_filter('bizpanda_items', 'opanda_register_email_locker', 1);
	...
	function opanda_email_locker_options($options, $id)
	{
		...
		if( 'custom-form' === $formType ) {
			...
			$strFieldsJson = isset($meta['opanda_fields'])
				? $meta['opanda_fields']
				: '';
			...
			$options['connectButtons']['vk'] = array(
		...
		$listId = ('database' === $service)
			? 'default'
			: opanda_get_item_option($id, 'subscribe_list', false);
	...
	add_filter('bizpanda_email-locker_item_options', 'opanda_email_locker_options', 10, 2);
	...
	function opanda_email_locker_assets($lockerId, $options, $fromBody, $fromHeader)
	{
		...
		if( !empty($options['opanda_has_mask']) ) {
			...
			wp_enqueue_script('jquery-maskedinput', OPANDA_BIZPANDA_URL . '/assets/js/jquery.maskedinput.min.js', array(
				'jquery',
				'opanda-lockers'
			), false, true);
		}
		...
		if( !empty($options['opanda_has_date']) ) {
			...
			wp_enqueue_script('jquery-moment', OPANDA_BIZPANDA_URL . '/assets/js/moment.js', array(
				'jquery',
				'opanda-lockers'
			), false, true);

			wp_enqueue_script('jquery-pikaday', OPANDA_BIZPANDA_URL . '/assets/js/pikaday.js', array(
				'jquery',
				'jquery-moment',
				'opanda-lockers'
			), false, true);
		...
		if( !empty($options['opanda_has_fontawesome']) ) {
			...
			wp_enqueue_style('factory-fontawesome', OPANDA_BIZPANDA_URL . '/assets/css/font-awesome/css/font-awesome.css');
		}

		if( !isset($options['opanda_subscribe_social_buttons']) || !$options['opanda_subscribe_social_buttons'] ) {
			return;
		}
	...
	add_action('bizpanda_request_assets_for_email-locker', 'opanda_email_locker_assets', 10, 4);
	...
	FactoryShortcodes000::register('OPanda_EmailLockerShortcode', $bizpanda);